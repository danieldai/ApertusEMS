...
import com.eltvpp.system.domain.ElectricPriceScheme;
import com.eltvpp.system.service.IElectricPriceSchemeService;
...
public class ElectricPriceSchemeController extends BaseController {
    ...
    public AjaxResult add(@RequestBody ElectricPriceScheme electricPriceScheme) {
        // 判断是否有相同年份的，一年只能配置一条
        if (electricPriceSchemeService.checkEffectYearUnique(publicService.getCurrentStation(), null) != null) {
    ...
    public AjaxResult edit(@RequestBody ElectricPriceScheme electricPriceScheme) {
        // 判断是否有相同年份的，一年只能配置一条
        if (electricPriceSchemeService.checkEffectYearUnique(publicService.getCurrentStation(), electricPriceScheme.getId()) != null) {
            return error("当前生效年份配置已经存在");
        }
    ...
    public AjaxResult changeStatus(@PathVariable Long id, @PathVariable Integer state) {
        ...
        if (state == 0 && electricPriceSchemeService.checkEffectYearUnique(publicService.getCurrentStation(), id) != null) {
            return error("当前生效年份配置已经存在");
        }
