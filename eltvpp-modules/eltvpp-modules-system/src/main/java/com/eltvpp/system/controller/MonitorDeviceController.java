...
import com.eltvpp.system.domain.SysCompany;
import com.eltvpp.system.service.*;
import com.eltvpp.common.core.enums.StationTypeGroupEnum;
...
import com.eltvpp.system.api.domain.FeignMonitorDevice;
...
import com.eltvpp.system.domain.jsonvo.AssociatedDevicesVo;
import com.eltvpp.system.service.*;
...
public class MonitorDeviceController extends BaseController {
    ...
    public AjaxResult listFusionGroup(MonitorDevice monitorDevice) {
        ...
        //站点类型：sys_station_type 枚举：1用电、2光伏、3用水、4流量、5储能、6计碳
    ...
    public AjaxResult add(@RequestBody MonitorDevice monitorDevice) {
        ...
        if (StringUtils.isEmpty(monitorDevice.getDeviceSn())) {
            //规则：站点编码_中文转英文+2位随机
            String sn = ChineseToPinyinUtils.ToFirstChar(monitorDevice.getDeviceName());
            monitorDevice.setDeviceSn(station.getStationSn() + "_" + sn + GenRandomUtils.GenRandomNumber(2));
        }
    ...
    public AjaxResult remove(@PathVariable Long[] ids) {
        return toAjax(monitorDeviceService.deleteMonitorDeviceByIds(new MonitorDevice(), ids));
    }
    //endregion
