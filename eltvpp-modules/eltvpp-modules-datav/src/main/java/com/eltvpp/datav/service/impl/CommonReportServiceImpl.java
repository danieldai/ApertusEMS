...
import com.eltvpp.datav.domain.dto.ReportConfigDto;
import com.eltvpp.datav.domain.vo.ChartQueryVo;
import com.eltvpp.datav.domain.vo.report.TableResult;
import com.eltvpp.datav.enums.DatavDateDimEnum;
import com.eltvpp.datav.service.CommonReportService;
...
public class CommonReportServiceImpl implements CommonReportService {
    ...
    private TableResult packageHeader(ReportQueryVo configVo) {
        ...
        for (ReportConfigDto item : configVo.getHeadConfigList()) {
            ...
            if (Objects.equals(configVo.getDateDim(), DatavDateDimEnum.DAY.getKey())) {
                ShardingQuery query = new ShardingQuery(item.getDeviceVarId(), 1, 1, dateToParamForDay);
            ...
            if (Objects.equals(configVo.getDateDim(), DatavDateDimEnum.MONTH.getKey())) {
                if (configVo.getIsFengGuData()) {
                    //峰谷数据
                    ShardingQuery query = new ShardingQuery(item.getDeviceVarId(), 2, 5, dateToParamForDay);
                    query.setIsCharge(configVo.getIsCharge());
                    shardingMonthList = shardingCommonService.continuityFengGuMonthDay(query);
                } else {
                    //基础数据
                    ShardingQuery query = new ShardingQuery(item.getDeviceVarId(), (configVo.getChangeType() < 5 ? 1 : 2), configVo.getChangeType(), dateToParamForDay);
            ...
            if (configVo.getIsFengGuData()) {
                ...
                for (int i = 0; i < configVo.getTimeList().size(); i++) {
                    ...
                    for (String legend : cloneList) {
                        packageResult(timeInfo.getKey(), item.getDeviceVarId() + "-" + legend, 0f, tempData);
                    }
                }

            } else {
                // 基础数据
                Map<String, Float> shardingMonthMap = shardingMonthList.stream().collect(Collectors.toMap(ShardingMonth::getFormattedDatetime, ShardingMonth::getDataValue));
                for (int i = 0; i < configVo.getTimeList().size(); i++) {
                    ChartQueryVo.TimeInfo timeInfo = configVo.getTimeList().get(i);
                    if (timeInfo.getValue() == null) {
                        continue;
                    ...
                    packageResult(timeInfo.getKey(), item.getDeviceVarId(), shardingMonthMap.getOrDefault(timeInfo.getKey(), 0f), tempData);
                }
            }
        }
    ...
    //region 组装结果数据
    private void packageResult(String date, String dataKey, Float dataValue, Map<String, Map<String, String>> resultMap) {
        if (resultMap.get(date) == null) {
            Map<String, String> map = new HashMap<>();
            map.put("date", date);
            map.put(dataKey, dataValue.toString());
            resultMap.put(date, map);
        } else {
            resultMap.get(date).put(dataKey, dataValue.toString());
        }
    }
    //endregion
