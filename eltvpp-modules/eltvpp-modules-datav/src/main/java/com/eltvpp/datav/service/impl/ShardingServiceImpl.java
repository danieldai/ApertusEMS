...
import com.eltvpp.datav.domain.ShardingDay;
import com.eltvpp.datav.domain.ShardingMonth;
import com.eltvpp.datav.domain.ShardingMonthAccumulate;
import com.eltvpp.common.core.constant.SecurityConstants;
import com.eltvpp.common.core.enums.DateFormatEnum;
import com.eltvpp.common.core.enums.sharding.ShardingDataTypeEnum;
import com.eltvpp.common.core.enums.sharding.XAxisTypeEnum;
import com.eltvpp.common.core.enums.vo.EnumSOVO;
...
import com.eltvpp.datav.domain.vo.ChartYAxisVo;
import com.eltvpp.datav.service.IShardingDayService;
import com.eltvpp.datav.service.IShardingMonthAccumulateService;
import com.eltvpp.datav.service.IShardingMonthService;
import com.eltvpp.datav.service.IShardingService;
import com.eltvpp.system.api.RemoteDictDataService;
import com.eltvpp.system.api.RemoteMonitorDeviceVarService;
import com.eltvpp.system.api.domain.FeignMonitorDeviceVar;
import com.eltvpp.system.api.domain.FeignSysPageConfig;
import com.eltvpp.system.api.domain.SysCommonDictData;
import com.eltvpp.system.api.domain.vo.FeignVariableConfigVo;
...
public class ShardingServiceImpl implements IShardingService {
    ...
    private List<String> packageMonthData(FeignVariableConfigVo variableVo, Integer xAxisType, String beginTime, String endTime, List<String> xAxis,
        ...
        if (variableVo.getDataType().equals(ShardingDataTypeEnum.MONTH_MAX.getKey())) {
            //最大值
            Float dataValue = shardingMonthService.selectShardingMonthMax(shardingMonth);
        ...
        if (variableVo.getDataType().equals(ShardingDataTypeEnum.MONTH_MIN.getKey())) {
            //最小值
            Float dataValue = shardingMonthService.selectShardingMonthMin(shardingMonth);
            ShardingMonth dto = new ShardingMonth();
            dto.setStatisticValue(dataValue);
            shardingMonthList.add(dto);
        }
        if (variableVo.getDataType().equals(ShardingDataTypeEnum.MONTH_AVG.getKey())) {
            //平均值
            Float dataValue = shardingMonthService.selectShardingMonthAvg(shardingMonth);
            ShardingMonth dto = new ShardingMonth();
            dto.setStatisticValue(dataValue);
            shardingMonthList.add(dto);
        }
        if (variableVo.getDataType().equals(ShardingDataTypeEnum.MONTH_LJZ.getKey())) {
            ...
            Float dataValue = shardingMonthService.selectShardingMonthDay(shardingMonth);
            ShardingMonth dto = new ShardingMonth();
            dto.setStatisticValue(dataValue);
            shardingMonthList.add(dto);
        }
        if (variableVo.getDataType().equals(ShardingDataTypeEnum.MONTH_CONT.getKey())) {
            ...
            ShardingMonth dto = new ShardingMonth();
            dto.setStatisticValue(Float.parseFloat(String.valueOf(dataValue)));
            shardingMonthList.add(dto);
        }
    ...
    public Map<String, Object> selectAccumulateGroupBySeasonalName(String[] deviceSns, String beginTime, String endTime) {
        ...
        for (FeignMonitorDeviceVar deviceVar : deviceVarList) {
            ...
            temp.put("倍率", deviceVar.getCoefficient());
            temp.put("总", total);
