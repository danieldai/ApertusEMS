...
import com.eltvpp.common.core.enums.vo.EnumSOCVO;
...
import com.eltvpp.system.api.domain.*;
import com.eltvpp.system.api.domain.vo.FeignVariableConfigPresetsUtils;
    ...
    public AjaxResult basicDataList(String[] deviceSns, String paramType, String dayTime, String monthTime, String yearTime) {
        ...
        for (FeignMonitorDeviceVar item : deviceVarList) {
                    ...
                    for (Integer head : heads) {
                        ...
                        for (ShardingMonthAccumulate day : list) {
                            if (day.getRecordHour().equals(head)) {
                                float dataValue = day.getAccuData();
                    ...
                    for (Integer head : heads) {
                        ...
                        for (ShardingDay day : list) {
                            if (day.getHour().equals(head)) {
                                float dataValue = day.getDataValue();
                                min = Math.min(min, dataValue);
                                max = Math.max(max, dataValue);
                                total += dataValue;
                                vo.setValue(dataValue + "");
                ...
                for (Integer head : heads) {
                    ...
                    for (ShardingMonth month : list) {
                        if (month.getMonth().equals(head)) {
                            float dataValue = month.getStatisticValue();
                            min = Math.min(min, dataValue);
                            max = Math.max(max, dataValue);
                            total += dataValue;
                            vo.setValue(dataValue + "");
            ...
            if (min == 99999999f) {
                ...
                vo3.setValue(FormatUtils.fmt2point(min));
            }
            midList.add(vo3);

            EnumSOVO vo4 = new EnumSOVO();
            vo4.setKey("最大值");
            vo4.setValue(FormatUtils.fmt2point(max));
            midList.add(vo4);

            EnumSOVO vo5 = new EnumSOVO();
            vo5.setKey("平均值");
            vo5.setValue(count == 0 ? "0.00" : FormatUtils.fmt2point(total / count));
            midList.add(vo5);

            EnumSOVO vo6 = new EnumSOVO();
            vo6.setKey("合计");
            vo6.setValue(FormatUtils.fmt2point(total));
            midList.add(vo6);
