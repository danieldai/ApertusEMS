<template>
  <a-layout class="layout" :class="{ mobile: appStore.hideMenu }">
    <a-layout>
      <a-layout>
        <a-layout-sider class="layout-sider-father" :width="55" :style="{
          background: isDark
            ? 'var(--color-bg-3)'
            : 'linear-gradient(rgb(var(--primary-6)), rgb(var(--primary-5)))',
        }">
          <div class="layout-sider-father-logo">
            <img alt="logo" src="@/assets/logo.png" :width="32" />
          </div>
          <div class="level-one-content">
            <div class="level-one-content-plugin">
              <div class="ibe-carbon-level-one">
                <template v-for="(item, index) in fatherMenu">
                  <div class="level-one-item" :class="{ 'level-one-item-select': index === fatherMenuActive }"
                    @click="fatherMenuActiveChange(index, item.path)">
                ...
                <div class="level-one-item" @click="goBI('bi')">
                  <div class="select-line"></div>
                  <i class="iconfont">
                    <icon-bar-chart />
                  </i>
        ...
        <a-layout-sider v-if="renderMenu" v-show="!hideMenu" class="layout-sider" breakpoint="xl" :collapsed="collapsed"
          :collapsible="true" :width="menuWidth" :style="{ zIndex: 101 }" :hide-trigger="true" @collapse="setCollapsed">
          <a-typography-title class="ykite-title" :heading="6">
            {{ title }}
          </a-typography-title>
        ...
        <a-layout class="layout-content" :style="paddingStyle">
          ...
          <a-layout-content style="padding: 0 15px 0 15px">
...
<script lang="ts" setup>
  ...
  const goBI = (name: any) => {
    ...
    let url = `${window.location.origin}/${name}?${qs.stringify(params)}`;
...
<style scoped lang="less">
  ...
  .layout-sider-father {
    ...
    /* background-image: linear-gradient(179deg, #015a45, #0a4537 37%, #054138 90%); */
