        ...
        <a-space direction="vertical" :size="16">
          <a-row :gutter="24">
            <a-col :span="12">
              ...
              <!-- 变量编号 -->
              <a-form-item field="deviceSn" :label="$t('power.energy.device.varCode')">
                <a-input disabled v-model="formData.varSn" />
              </a-form-item>

              <!-- 用电设备 -->
              <a-form-item field="deviceKey" :label="$t('power.energy.device.electricityDevice')">
                <a-tree-select allow-search v-model="formData.deviceKey" :data="deviceData" :placeholder="$t('power.energy.device.selectElectricityDevice')"
              ...
              <a-form-item field="varMapId" :label="$t('power.energy.device.varAttribute')">
                <a-cascader v-model="formData.varMapId" :options="varMapSnData" expand-trigger="hover" allow-search
                  @change="handleVarMapChange" @clear="handleVarMapClear" :placeholder="$t('power.energy.device.selectVarAttribute')" />
              </a-form-item>
              <!-- 变量单位 -->
              ...
              <a-form-item field="variableType" :label="$t('power.energy.device.varType')">
                <a-select v-model="formData.variableType" :placeholder="$t('power.energy.device.selectVarType')" allow-clear>
...
<!--              </a-form-item>-->
              ...
              <a-form-item field="stopFlag" :label="$t('global.status')">
                <a-switch :checked-value="0" :unchecked-value="1" v-model="formData.stopFlag">
                  <template #checked>{{ $t('global.enable') }}</template><template #unchecked>{{ $t('global.forbidden') }}</template></a-switch>
              </a-form-item>

              <a-divider orientation="left" style="margin-top: 40px;">{{ $t('power.energy.device.dataSource') }}</a-divider>
              <!-- 来源类型 -->
              ...
              <div v-if="formData.origin == 1">
                <!-- 通讯设备 -->
                <a-form-item field="comDeviceId" :label="$t('power.energy.device.communication')" :validate-trigger="['change', 'blur', 'input']">
                ...
                <!-- 信息体地址 -->
                <a-form-item v-if="formData.connectModel=='IEC104'" field="messageAddress" :label="$t('power.energy.device.infoAddress')" :validate-trigger="['change', 'blur', 'input']">
                  <a-input-number v-model="formData.messageAddress" />
                </a-form-item>
                <a-form-item v-else-if="formData.connectModel=='MODBUSTCP' || formData.connectModel== 'MODBUSRTU'" field="registerAddress" :label="$t('power.energy.device.registerAddress')" :validate-trigger="['change', 'blur', 'input']">
                  <a-input-number v-model="formData.registerAddress" />
                </a-form-item>
                <a-form-item v-else-if="formData.connectModel=='DLT645' ||  formData.connectModel== 'TianGang'" field="dataAddress" :label="$t('power.energy.device.dataIdentifier')" :validate-trigger="['change', 'blur', 'input']">
                  <a-select v-model="formData.dataAddress" v-if="formData.connectModel=='DLT645'" :placeholder="$t('global.pleaseSelect')" allow-clear>
                    <a-option v-for="dict in sys_config_variable_type"  :key="dict.value" :label="dict.label"
                      :value="Number(dict.value)"></a-option>
                  </a-select>
                  <a-input v-if="formData.connectModel=='TianGang'" v-model="formData.dataAddress" :placeholder="$t('global.please')" />
                </a-form-item>
                <!-- 读写类型 -->
                <a-form-item field="rw" :label="$t('power.energy.device.readWriteType')" :validate-trigger="['change', 'blur', 'input']">
                  <!-- 1只读 2只写 3读写 -->
                  ...
                  <a-radio-group v-model="formData.rw">
                    <a-radio v-for="dict in sys_rw" :key="dict.value" :value="Number(dict.value)">{{ dict.label
                    }}</a-radio>
                  </a-radio-group>
                </a-form-item>
              </div>
                ...
                <a-form-item field="computeFormula" :label="$t('power.energy.device.formula')">
                  <a-grid :cols="5" :colGap="20" :rowGap="5" class="grid-calculator">
                    ...
                    <a-grid-item class="calculator-item">
                      <a-button type="outline" status="success" @click="handleVarTypeSelect(1)">{{ $t('pv.alarm.category.analog') }}</a-button>
                    </a-grid-item>
                    <a-grid-item class="calculator-item">
                    ...
                    <a-grid-item class="calculator-item">
                      <a-button type="outline" status="success" @click="handleVarTypeSelect(2)">{{ $t('pv.alarm.category.state') }}</a-button>
                    </a-grid-item>
                    <a-grid-item class="calculator-item">
                    ...
                    <a-grid-item class="calculator-item">
                      <a-button type="primary" status="danger" @click="handleCalculatorClear">{{ $t('messageBox.tab.button') }}</a-button>
                    </a-grid-item>
                    <a-grid-item class="calculator-item">
            ...
            <a-col :span="12">
              ...
              <a-form-item field="dataType" :label="$t('power.energy.device.dataType')">
                <a-select v-model="formData.dataType" :placeholder="$t('power.energy.device.selectDataType')" allow-clear>
              ...
              <a-form-item field="initValue" :label="$t('power.energy.device.inAssignment')">
                <a-input-number v-model="formData.initValue" allow-clear :placeholder="$t('power.energy.device.pleaseNum')">
                </a-input-number>
              </a-form-item>

              <!-- 数据基值 -->
              <a-form-item field="baseValue" :label="$t('power.energy.device.dataBaseValue')">
                <a-input-number v-model="formData.baseValue" allow-clear :placeholder="$t('power.energy.device.pleaseNum')">
                </a-input-number>
              </a-form-item>
              ...
              <a-form-item field="saveCycle" :label="$t('power.energy.device.savePeriod')">
                <a-select v-model="formData.saveCycle" :placeholder="$t('power.energy.device.selectSavePeriod')" allow-clear>
              ...
              <a-form-item field="isAccumulation" :label="$t('power.energy.device.isCumulative')">
                <a-switch :checked-value="1" :unchecked-value="0" v-model="formData.isAccumulation">
                  <template #checked>{{ $t('global.yes') }}</template><template #unchecked>{{$t('global.no')}}</template></a-switch>
              </a-form-item>

              <!-- 累积类型 -->
              <a-form-item v-if="formData.isAccumulation == 1" field="accumulationType" :label="$t('power.energy.device.cumulativeType')">
                <!-- 1小时累积 2天累积 -->
              ...
              <a-divider orientation="left" style="margin-top: 40px">{{ $t('power.energy.device.dataFilter') }}</a-divider>
              <!-- 数据过滤 -->
              <a-form-item field="isDataFilter" :label="$t('power.energy.device.dataFilter')">
                <a-switch :checked-value="1" :unchecked-value="0" v-model="formData.isDataFilter">
                  <template #checked>{{ $t('pv.alarm.setting.open') }}</template><template #unchecked>{{ $t('pv.alarm.setting.close') }}</template></a-switch>
              </a-form-item>
        ...
        <div class="actions">
          <a-space>
            ...
            <a-button type="primary" :disabled="submitLoading" :loading="submitLoading"
              @click="onSubmitClick">{{ $t('global.save') }}</a-button>
          </a-space>
        </div>
      </a-form>
    </a-spin>
...
const { sys_data_type, sys_save_cycle, sys_data_origin, sys_rw, sys_deletion_type,sys_accumulate_type, sys_config_variable_type } = proxy?.useDict("sys_data_type", "sys_save_cycle", "sys_data_origin", "sys_rw", "sys_deletion_type","sys_accumulate_type",'sys_config_variable_type');
//******* 这里编写动态获取下拉列表 end ******
//通讯设备
...
const formRule = {
  ...
  deviceSn: [{ required: true, message: "请输入变量编号" }],
