    ...
    <a-card class="content">
      ...
      <a-table row-key="id" ref="tableRef" :loading="loading" :pagination="false"
        ...
        <template #operate="{ record }">
          ...
          <a-button v-if="record.isSystem == 0" size="small" type="text" status="success" @click="handleUpdate(record)">{{ $t('global.edit') }}</a-button>
          <a-button v-if="record.isSystem == 0" size="small" type="text" status="danger" @click="handleDelete(record)">
            {{ $t('global.delete') }}
          </a-button>
        </template>
      </a-table>
          ...
          <a-col :span="24">
            <a-form ref="formRef" auto-label-width :model="formDrawer.formModel" label-align="right"
              ...
              <a-form-item field="groupName" :label="$t('global.groupName')">
                <a-input :placeholder="$t('menu.diagram.project.power.list.projectNamePlaceholder')" v-model="formDrawer.formModel.groupName" />
              </a-form-item>
              ...
              <a-form-item field="remark" :label="$t('pv.device.remark')">
                <a-textarea :placeholder="$t('pv.device.placeholder')" :max-length="200" v-model="formDrawer.formModel.remark"
                  allow-clear />
              </a-form-item>
...
const handleAdd = async () => {
  //获取是否有多级
  let res = await dictDataMultiLevel("sys_group_map", props.mapId);
  if (res.code == 200) {
    if (res.data == 1) {
