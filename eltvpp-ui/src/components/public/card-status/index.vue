...
const iconStyle = computed(() => ({
  fontSize: '50px',
  color: chartInfo.value.statusInfo.status === 1 ? '#f5222d' : '#52c41a'
}));
...
// 监听 webSocket 数据
watch(data, (newVal) => {
  if (newVal.key && newVal.key === chartInfo.value.statusInfo.varSn) {
...
onMounted(async () => {
  ...
  if (chartInfo.value.statusInfo.varSn) {
    sendData({
      action: 'subscribe',
      varSn: chartInfo.value.statusInfo.varSn
    });
  }
});
