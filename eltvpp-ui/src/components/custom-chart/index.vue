...
const shouldRenderChart = computed(() => {
  // renderChart 为 true 且 options 对象中存在 xAxis 并且其第一个元素的数据长度大于 0
  return renderChart.value && (props.options?.xAxis?.[0]?.data?.length > 0 || props.options?.yAxis?.[0]?.data?.length > 0);
});
