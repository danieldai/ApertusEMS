<template>
  ...
  <a-row :gutter="[0, 12]">
    <a-col v-for="(item,index) in form.varList" :key="index">
      <a-card :title="`数据区${index+1}`">
        ...
        <a-form ref="formRef" :model="item" auto-label-width>
          ...
          <!--堆线图、K线图不显示、-->
          <template v-if="![4, 7].includes(defaultVal.chartType)">
            <!-- 状态量-->
            <template v-if="![13].includes(defaultVal.chartType)">
            ...
            <!--饼图、状态量不展示-->
            <template v-if="![6,13].includes(defaultVal.chartType)">
            ...
            <template v-if="![12,6,8,9].includes(defaultVal.chartType)">
              <!--ICON-->
              ...
              <a-form-item v-if="[14].includes(defaultVal.chartType)" field="color" label="ICON颜色"
                ...
                />
                <icon-close-circle class="delete-icon" @click="handleIconColor(item, index)"/>
              </a-form-item>
    ...
    <a-col v-if="![14, 4, 7, 8, 9, 13].includes(defaultVal.chartType)">
      <a-button type="primary" @click="handleAddDataArea">添加数据区</a-button>
    </a-col>

    <!--弹窗-->
    <select-var-modal :visible="selectVarVisible" :varType="defaultVal.chartType==13?2:1" :is-accumulation="defaultVal.chartType==4?1:0" @cancel="handleModalClose"
...
const {
  ...
  datav_change_type,
  datav_percentage
} = proxy?.useDict("sys_config_variable_type", "datav_equip_type", "datav_state_type", "datav_storage_type", "datav_change_type", "datav_percentage");
...
// 删除icon颜色
const handleIconColor = async (item: any, index: number) => {
  form.value.varList[index].iconColor = "";
  await nextTick();
  if (item.iconColor == "#FF0000") {
    form.value.varList[index].iconColor = "";
  }
};
