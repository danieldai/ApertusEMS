      ...
      <a-form-item field="staticType" label="数据类型" v-if="[15,16].includes(defaultVal.chartType)">
        <a-select v-model="form.staticType" placeholder="请选择" allow-clear>
         ...
         <template v-if="defaultVal.chartType==15">
           <a-option v-for="(item,index) in sys_trigger_status" :key="index" :value="Number(item.value)">{{ item.label }}
           </a-option>
         </template>
        </a-select>
      </a-form-item>

      <template v-if="![13,14,11,4,12,6,8,9,15,16,17,18].includes(defaultVal.chartType)">
          ...
          <a-row :gutter="[0,16]">
            ...
            <a-col :span="24">
              <a-select v-model="form.dateDim" placeholder="请选择" allow-clear style="width: 400px"
                ...
                <a-option v-for="(item,index) in datav_date_dim" :value="Number(item.value)" :key="index">
                  {{ item.label }}
                </a-option>
              </a-select>
            </a-col>
        ...
        <!--全日期-->
        <a-form-item field="isFullDate" label="全日期" v-if="![14, 7,10].includes(defaultVal.chartType)">
          <!--          <a-radio-group v-model="form.isFullDate">-->
