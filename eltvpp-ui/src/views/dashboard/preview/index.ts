...
const calculateNewPosition = (pageConfig: any, layout: { w: number, h: number }) => {
	...
	pageConfig.forEach((item: any, index: number) => {
		if (index === pageConfig.length - 1 || item.y > (pageConfig[index + 1]?.y || 0)) {
			// 当前元素属于最后一行，或之后无元素（已是最后一行）
			lastRowY = item.y;
			lastRowMaxX = Math.max(lastRowMaxX, item.x + item.w);
		}
	});
