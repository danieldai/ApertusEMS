      ...
      <a-row style="margin-bottom: 16px">
        <a-col>
          ...
          <a-table ref="tableRef" row-key="id" :loading="loading" v-bind:hide-expand-button-on-empty="true"
            ...
            <!-- 状态 -->
            <template #deviceDayRuntimeStatus="{ record }">
              <div v-if="record.isGroup == 0">
                <span v-if="record.deviceDayRuntime.status === 1" class="circle-device"></span>
...
import { StationTypeEnum, changeDeviceStatus, getPVTab, listFusionGroup } from '@/api/system/device';
...
const columns = computed<TableColumnData[]>(() => [
  ...
  {
    title: "状态",
    dataIndex: "deviceDayRuntime.status",
    slotName: "deviceDayRuntimeStatus",
    align: "center",
    width: 100,
  },
  {
...
const handleOperateModelOk = async () => {
  let result = await changeDeviceStatus(operateModalModel.value.id, operateModalModel.value.value);
  notification(result);
  if (result.code == 200) {
