    ...
    <a-card class="content">
      ...
      <a-table row-key="id" ref="tableRef" :loading="loading" :pagination="pagination" :scroll="{ x: 1650 }"
        ...
        <template #operate="{ record }">
          <a-button size="small" type="text" status="warning" @click="handleConfirm(record)">{{
              $t('global.confirm')
            }}
          </a-button>
          ...
          <a-button size="small" type="text" @click="handleCreateOrder">
            <template #icon>
              <icon-plus/>
            </template>
...
const detailDataList = computed<DescData[]>(() => {
  const result: DescData[] = [
    ...
    {
      label: '报警状态',
      value: getDictLabel("sys_trigger_status", formDrawer.value.formModel.triggerStatusName) || '-',
    },
    {
      label: '解决人员',
