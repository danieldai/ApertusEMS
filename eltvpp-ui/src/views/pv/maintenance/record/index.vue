    ...
    <a-card class="content">
      ...
      <a-table :scroll="{ x: 1110 }" row-key="id" ref="tableRef" :loading="loading"
        ...
        <template #operate="{ record }">
          ...
          <a-button size="small" type="text" status="success" @click="handleDetail(record.id)">{{ $t('global.detail') }}</a-button>
        </template>
      </a-table>
      <!-- 表格内容 end -->
      ...
      <!-- 添加/编辑维保 -->
      <a-drawer :width="700" :esc-to-close="false" :visible="formDrawer.visible" @ok="handleSubmitForm"
        ...
        <a-spin :loading="formDrawer.loading" style="width: 100%;height: auto;" :tip="$t('global.loading')">
          <a-form ref="formRef" auto-label-width :model="formDrawer.formModel" label-align="right"
            ...
            <a-form-item field="stationType" :label="$t('pv.maintenance.list.siteType')">
              <a-select disabled v-model="formDrawer.formModel.stationType" :placeholder="$t('pv.maintenance.list.pleaseSelectSite')" allow-clear>
                <a-option v-for="dict in sys_station_type" :key="dict.value" :label="dict.label"
            ...
            <!-- 关联站点 -->
            <a-form-item field="stationId" :label="$t('pv.maintenance.record.relatedSite')" :content-flex="false" :merge-props="false">
              <a-tree-select allow-search :allow-clear="false" v-model="formDrawer.formModel.stationId"
                :data="renderStationData" @change="handleStationChange" :placeholder="$t('pv.maintenance.record.ElectSite')" :fieldNames="{
          key: 'id', title: 'deptName', children: 'children'
            ...
            <a-form-item field="deviceIdFor" :label="$t('pv.maintenance.record.relatedDevice')">
              <a-space direction="vertical" fill :size="16">
                ...
                <a-table row-key="value" :bordered="{ wrapper: true, cell: true }" :pagination="false"
                  :data="formDrawer.formModel.deviceIdFor" :columns="(deviceColumns as TableColumnData[])"
                  v-bind:hide-expand-button-on-empty="true">
                  <template #operate="{ rowIndex }">
                    <a-button size="small" type="primary" status="danger"
                      @click="handleDeviceDel(rowIndex)">{{ $t('global.remove') }}</a-button>
                  </template>
                </a-table>
              </a-space>
            </a-form-item>
            ...
            <!-- 维保人员 -->
            <a-form-item field="userIds" :label="$t('pv.maintenance.record.mPerson')" :validate-trigger="['change', 'input']">
              ...
              <a-descriptions :align="{ label: 'right' }" :label-style="{ color: 'rgb(var(--color-neutral-10))' }"
                  ...
                  <span v-else>
                    <a-table :bordered="{ wrapper: true, cell: true }" :data="formDrawer.formModel.deviceIdFor"
                      ...
                      <template #columns>
                        ...
                        <a-table-column :title="$t('pv.maintenance.record.deviceCode')" data-index="tip">
                ...
                <a-descriptions-item label="图片描述:" :label="`${$t('pv.maintenance.record.mContent)')}:`">
                  ...
                  <a-upload v-else-if="formDrawer.formModel.filesImage && formDrawer.formModel.filesImage.length > 0"
                    disabled :download="true" list-type="picture-card" :file-list="formDrawer.formModel.filesImage"
        ...
        <a-row style="margin-bottom: 16px">
          <a-col style="text-align: center;">
            ...
            <a-table ref="tableDeviceRef" :bordered="{ wrapper: true, cell: true }" :scroll="{ y: 500 }"
              :pagination="false" :data="modalModel.renderData" :columns="(deviceModalColumns as TableColumnData[])"
              ...
              <template #operate="{ record }">
                <a-button v-if="record.isGroup == 0" size="small" type="primary"
                  @click="handleDeviceSelect(record)">选择</a-button>
              </template>
            </a-table>
          </a-col>
        </a-row>
...
import { addMaintenance, delMaintenance, getMaintenance, listMaintenance, updateMaintenance } from '@/api/system/maintenance';
import { JsonCommonPlusVo, JsonCommonVo, getStationList } from '@/api/public';
...
//设备列表-列
const deviceColumns = computed<TableColumnData[]>(() => [
  {
    title: "设备名称",
    dataIndex: "name",
    align: "left",
...
const deviceModalColumns = computed<TableColumnData[]>(() => [
  {
    title: "设备名称",
...
const generateFormDrawerModel = () => {
  return {
    ...
    formModel: {
      ...
      /**设备列表 */
      deviceIdFor: [] as JsonCommonPlusVo[],
      stopFlag: 0,
...
const handleUpdate = async (id: number) => {
  try {
    ...
    if (res.code == 200) {
      ...
      handleStationChange(res.data.stationId);
    }
...
const handleDeviceSelect = (record: any) => {
  let info: any = formDrawer.value.formModel.deviceIdFor.find(item => item.tip == record.deviceSn);
  if (!info) {
...
const handleStationChange = async (val: any) => {
  ...
  // formDrawer.value.formModel.deviceIdFor = [] as JsonCommonPlusVo[];
}
