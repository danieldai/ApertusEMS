  ...
  <div>
    <a-card class="content">
      ...
      <a-row align="center">
        ...
        <a-col :span="'80px'" style="text-align: right;padding-bottom: 12px">
          <a-space :size="18">
      ...
      <!-- 添加右弹层 start -->
      <a-drawer :width="1000" :esc-to-close="false" :visible="formDrawer.visible" @ok="handleSubmitForm"
          ...
          <a-row>
            ...
            <a-col>
              <a-table :loading="modalModel.loading" :scroll="{ y: 500 }" row-key="id"
                       :bordered="{ wrapper: true, cell: true }" :columns="(userTabColumns as TableColumnData[])"
...
const handleOrderStatus = async (val: any) => {
  formDrawer.value.loading = true;
  try {
    let param = formDrawer.value.formModel;
    if (val == 1) {
...
const fetchLeaveMessageData = async () => {
  let resMsg = await listLeaveMessage({
    tableName: "devops_order",
    recordId: formDrawer.value.formModel.id,
    pageSize: 10000,
    pageNum: 1,
