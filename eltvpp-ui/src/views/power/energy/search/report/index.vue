  ...
  <div>
    <a-card>
      <div class="action-bar-view">
        ...
        <a-space>
          <time-bar @change="timeChange" is-current-date-disabled :date-unit="props.reportType === 2 ? ['month','year']: ['day','month','year']"/>
          <a-select v-model="searchForm.changeType" :style="{width:'220px'}" placeholder="请选择" v-if="props.reportType === 1 && timeParams.timeUnit !== 0">
    ...
    <a-button type="primary" @click="handleConfigView">
      配置模板
    </a-button>
...
const fetchTabsData = async () => {
  setLoading(true);
  try {
    const res = await getTemplateData({reportType: props.reportType});
...
const handleTableColumnsWidth = (arr:any[]) => {
  arr.forEach(item => {
    ...
    if (item.children) {
      handleTableColumnsWidth(item.children)
    }
  });
};
/**
...
const myExport = async () => {
  ...
  const xlsxName = `${result.reportName} ${dayjs().format('YYYY-MM-DD HH:mm:ss')}`;
  await tableExport({column, data}).download(xlsxName);
  setLoading(false);
};
/**
 * @desc 跳转配置模板页
...
const handleConfigView = () => {
  router.push({
    path: '/power/energy/search/report/template',
    query: {
