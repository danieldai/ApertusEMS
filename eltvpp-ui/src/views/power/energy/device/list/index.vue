  ...
  <div>
    <a-card class="content">
            ...
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="groupId" label="设备分组">
                  <a-tree-select allow-clear allow-search v-model="searchModel.groupId" :fieldNames="{
          key: 'id', title: 'groupName', children: 'children'
        }" :data="catalogData" placeholder="请选择设备分组"></a-tree-select>
                </a-form-item>
              </a-col>
              <a-col :span="8">
      ...
      <a-table ref="tableRef" row-key="id" :loading="loading" v-bind:hide-expand-button-on-empty="true"
        :pagination="false" :bordered="{ wrapper: true, cell: true }" :columns="(columns as TableColumnData[])"
        :data="renderData" :scroll="{ x: 1370 }">
        <!-- 设备名称 -->
        ...
        <template #ratedVol="{ record }">
          <span v-if="record.isGroup == 0">{{ record.ratedVol }}V</span>
        </template>
        <!-- 额定电流 A -->
        ...
        <template #operate="{ record }">
          ...
          <a-button v-if="record.isGroup == 0" size="small" type="text" status="danger"
            @click="handleDelete(record)">{{ $t('global.delete')  }}</a-button>
          <a-button v-if="record.isGroup == 0" size="small" type="text"
            @click="handleToVariable(record.id)">{{$t('power.energy.device.variable')}}</a-button>
          <!-- </a-space> -->
        </template>
      </a-table>
...
const { sys_electric_type } = proxy?.useDict("sys_electric_type");
...
const handleToVariable = (val: number) => {
  router.push({
    path: "/power/energy/device/listVar",
    query: {
      deviceId: val,
...
const handleAdd = () => {
  router.push({
    path: "/power/energy/device/create",
    query: {
      stationType: StationTypeEnum.power,
      redirect: router.currentRoute.value.fullPath
    }
...
/** 导出按钮操作 */
const handleExport = () => {
  download("/system/device/export?stationType=" + StationTypeEnum.power, {
  }, `用电设备-${dayjs().format("YYYY-MM-DD HH:mm:ss")}.xlsx`);
}
/**
 * 编辑数据
 * @param row 数据行
...
const handleUpdate = async (record: any) => {
  router.push({
    path: "/power/energy/device/update",
    query: {
      stationType: StationTypeEnum.power,
      bizId: record.id,
      redirect: router.currentRoute.value.fullPath
