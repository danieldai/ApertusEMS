  ...
  <div>
    <a-card class="content">
      ...
      <a-table row-key="id" :loading="loading" :pagination="pagination" :bordered="{ wrapper: true, cell: true }"
               :scroll="{ x: 1300 }" :columns="(columns as TableColumnData[])" :data="renderData"
          ...
          <a-row :gutter="12" class="row-mp-30">
            <a-col :span="12">
              ...
              <a-form-item field="entName" :label="$t('manage.system.enterprise')"
                ...
                <a-input v-model="formDrawer.formModel.entName" allow-clear/>
              </a-form-item>

              <!-- 企业类型 -->
              <a-form-item field="entType" :label="$t('manage.system.enterpriseType')">
                <a-select v-model="formDrawer.formModel.entType" :placeholder="$t('manage.system.selectEnterpriseType')"
                          allow-clear>
              ...
              <a-form-item field="entLogo" :label="$t('manage.system.enterpriseLogo')">
                <a-upload :limit="1" :key="fileLogoListKey" list-type="picture-card" action="/"
                          :default-file-list="formDrawer.fileLogoList"
              ...
              <a-form-item field="creditCode" :label="$t('manage.system.creditCode')"
                           :validate-trigger="['change', 'blur', 'input']">
                <a-input v-model="formDrawer.formModel.creditCode" allow-clear/>
              </a-form-item>

              <!-- 企业法人 -->
              <a-form-item field="legalPerson" :label="$t('manage.system.legalPerson')"
                           :validate-trigger="['change', 'blur', 'input']">
                <a-input v-model="formDrawer.formModel.legalPerson" allow-clear/>
              </a-form-item>
              ...
              <a-form-item field="linkMobile" :label="$t('manage.system.contactPhone')"
                           :validate-trigger="['change', 'blur', 'input']">
                <a-input v-model="formDrawer.formModel.linkMobile" allow-clear
            ...
            <a-col :span="12">
              ...
              <a-form-item field="registeredCapital" :label="$t('manage.system.registeredCapital')"
                           :validate-trigger="['change', 'blur', 'input']">
                <a-input-number v-model="formDrawer.formModel.registeredCapital" allow-clear>
                  <template #append>
                    万元
                  </template>
                </a-input-number>
...
const fileLogoListKey = ref<number>(0);
...
const handleUpdate = async (record: any) => {
  let result = await getEnterprise(record.id);
  if (result.code == 200) {
    result.data.entType += '';
    result.data.stopFlag += '';
...
const handleDelete = (record: any) => {
  ...
  operateModalModel.value.name = record.postName;
...
const handleOperateModelOk = async () => {
  ...
  if (operateModalModel.value.type == 0) {
    let info = await getEnterprise(operateModalModel.value.id);
    if (info.code == 200) {
...
const fetchData = async () => {
  setLoading(true);
  try {
    const res = await listEnterprise({
