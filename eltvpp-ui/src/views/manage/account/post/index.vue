<template>
  <div>
    <a-card class="content">
            ...
            <a-row :gutter="24">
              <a-col :span="22">
                <a-form-item field="postName" :label="$t('manage.account.post.jobTitle')">
                  <a-input v-model="searchParams.postName" :placeholder="$t('manage.account.post.jobTitlePlaceholder')" allow-clear />
                </a-form-item>
              </a-col>
            </a-row>
          </a-form>
      ...
      <a-table row-key="id" :loading="loading" :bordered="{ wrapper: true, cell: true }" :pagination="pagination"
        ...
        <template #operate="{ record }">
          ...
          <a-button size="small" type="text" status="success" @click="editChannelClick(record.id)">{{$t('global.edit')}}</a-button>
          <a-button size="small" type="text" status="danger" @click="handleModalClick('删除', record.id,record.id)">{{$t('global.delete')}}
          </a-button>
        </template>
    ...
    <!-- 添加右弹层 start -->
    <a-drawer :width="750" :visible="drawerVisible" @ok="handleFormOK" @cancel="handleFormCancel" :mask-closable="false">
      <template #title>
        ...
        <a-col :span="24">
          <a-form ref="formRef" :model="formData" label-align="right" :rules="formRules" auto-label-width>
            <a-form-item field="postName" :label="$t('manage.account.post.jobTitle')" :validate-trigger="['blur']">
              <a-input v-model="formData.postName" :max-length="20" allow-clear :placeholder="$t('manage.account.post.titleMaximumPlaceholder')" />
            </a-form-item>

            <a-form-item field="postSn" :label="$t('manage.account.post.postCode')" :validate-trigger="['blur']">
              <a-input v-model="formData.postSn" :max-length="20" allow-clear :placeholder="$t('manage.account.post.postCodePlaceholder')" />
            </a-form-item>
            ...
            <a-form-item field="status" :label="$t('manage.account.post.positionStatus')">
              <a-switch v-model="formData.stopFlag" :checked-value="checkedValue"
                :unchecked-value="unCheckedValue"><template #checked>{{$t('global.normal')}}</template><template
                  #unchecked>{{$t('global.outOfService')}}</template></a-switch>
            </a-form-item>
