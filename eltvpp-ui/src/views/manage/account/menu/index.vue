          ...
          <a-row :gutter="24">
            <a-col :span="22">
              <a-form-item field="menu" :label="$t('manage.account.menu.menuName')">
                <a-input v-model="searchParams.menuName" :placeholder="$t('manage.account.menu.menuNamePlaceholder')" allow-clear />
              </a-form-item>
            </a-col>
          </a-row>
        </a-form>
...
<!--          <a-button @click="setMenuClick"> {{// $t('manage.account.menu.enterprisemenu')}}（暂不可用） </a-button>-->
    ...
    <!-- 表格 start-->
    <a-table row-key="id" ref="tableRef" v-bind:hide-expand-button-on-empty="true" :loading="loading" :pagination="false"
      ...
      <template #visible="{ record }">
        <a-space v-if="record.visible === 0">
          <a-tag color="rgb(var(--green-6))">{{$t('manage.account.menu.menuReveal')}}</a-tag>
        </a-space>
        <a-space v-if="record.visible === 1">
          <a-tag color="rgb(var(--red-5))">{{$t('manage.account.menu.menuHidden')}}</a-tag>
        </a-space>
        <a-space v-else-if="record.visible === 2"> - </a-space>
      </template>
      <template #operate="{ record }">
        <a-button size="small" type="text" status="success" @click="handleUpdate(record)">{{$t('global.edit')}}</a-button>
        <a-button size="small" type="text" status="danger" @click="handleDelete('删除', record.id)">{{$t('global.delete')}}</a-button>
      </template>
    </a-table>
    <!-- 表格 end-->

    <!-- 编辑右弹层 start -->
    <a-drawer :width="750" :visible="createDrawerVisible" @ok="createDrawerHandleOk" @cancel="createDrawerHandleCancel"
      :mask-closable="false">
      <template #title> {{$t('manage.account.menu.addMenu')}} </template>
      <a-row :gutter="12">
        <a-col :span="24">
          <a-form ref="formRef" :model="formData" class="form" :rules="formRules" label-align="right" auto-label-width>
            <!-- 模块类型 -->
            <a-form-item field="menuModel" :label="$t('manage.account.menu.moduleType')" :validate-trigger="['change', 'blur']">
              <a-select v-model="formData.menuModel" :placeholder="$t('manage.account.menu.moduleTypeSelect')" allow-clear >
            ...
            <a-form-item field="menuName" :label="$t('manage.account.menu.menuName')">
              <a-input v-model="formData.menuName" :max-length="50" allow-clear :placeholder="$t('manage.account.menu.menuNameTip')" />
            </a-form-item>
            <a-form-item field="parentId" :label="$t('manage.account.menu.parentDirectory')">
              <a-tree-select allow-search v-model="formData.parentId" :data="parentList" :placeholder="$t('manage.account.menu.rootDirectory')"
            ...
            <a-form-item field="path" :label="$t('manage.account.menu.routingAddress')" v-if="formData.menuType === 'C' || formData.menuType === 'M'">
              <template #extra>
                <span>
            ...
            <a-form-item field="requestUrl" :label="$t('manage.account.menu.componentPath')" v-if="formData.menuType === 'C' || formData.menuType === 'M'">
              <template #extra>
                <span>
                  <i class="icon" :style="{ color: 'rgb(var(--primary-6))' }"><icon-exclamation-circle-fill /></i>
            ...
            <a-form-item field="requestQuery" :label="$t('manage.account.menu.routeParameter')" v-if="formData.menuType !== 'F'">
              <template #extra>
                <span>
                  <i class="icon" :style="{ color: 'rgb(var(--primary-6))' }"><icon-exclamation-circle-fill /></i>
                  {{$t('manage.account.menu.routeParameterTip')}}
                </span>
              </template>
            ...
            <a-form-item field="visible" :label="$t('manage.account.menu.displayStatus')" v-if="formData.menuType != 'F'">
              <template #extra>
                <span>
                  <i class="icon" :style="{ color: 'rgb(var(--primary-6))' }"><icon-exclamation-circle-fill /></i>
              ...
              <a-switch :checked-value="checkedValue" :unchecked-value="unCheckedValue"
                v-model="formData.visible"><template #checked>{{$t('global.reveal')}}</template><template #unchecked>{{$t('global.hidden')}}</template></a-switch>
            </a-form-item>
            <a-form-item field="stopFlag" :label="$t('manage.account.menu.menuStatus')" v-if="formData.menuType != 'F'">
              <template #extra>
                <span>
                  <i class="icon" :style="{ color: 'rgb(var(--primary-6))' }"><icon-exclamation-circle-fill /></i>
              ...
              <a-switch :checked-value="checkedValue" :unchecked-value="unCheckedValue"
                v-model="formData.stopFlag"><template #checked>{{$t('global.normal')}}</template><template #unchecked>{{$t('global.outOfService')}}</template></a-switch>
            </a-form-item>
          </a-form>
        </a-col>
      </a-row>
    ...
    <a-modal width="400px" v-model:visible="modalVisible" class="modal-box">
      ...
      <div style="text-align: center">
        {{$t('manage.account.menu.deleteTip')}}
      </div>
      <template #footer>
        <div style="text-align: center">
          <a-space>
...
import { listMenu, getMenu, addMenu, updateMenu, delMenu } from "@/api/manage/account/menu";
 ...
 * @param bizId 业务ID
 */
const handleDelete = (title: string, bizId: number) => {
...
const handleDeleteOk = async () => {
  if (modalBizId && modalBizId.value > 0) {
    let res = await delMenu(modalBizId.value);
    notification(res);
