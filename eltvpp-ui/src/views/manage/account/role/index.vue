<template>
  <a-card class="content">
    ...
    <a-table row-key="id" :loading="loading" :bordered="{ wrapper: true, cell: true }" :pagination="pagination"
      ...
      <template #operate="{ record }">
        ...
        <a-button v-if="!record.admin" size="small" type="text" status="success"
          @click="handleRoleUpdate(record.id)">{{$t('global.edit')}}</a-button>
        <a-button v-if="!record.admin" size="small" type="text" status="danger"
          @click="handleModalClick('删除', record.id, 0, 1)">{{$t('global.delete')}}</a-button>
        <a-button v-if="!record.admin" size="small" type="text" status="success"
          @click="handleUpdateDept(record.id, record.dataScope)">{{$t('manage.account.role.dataPermission')}}</a-button>
        <a-button v-if="!record.admin" size="small" type="text" status="success"
          @click="handleViewAuthUserClick(record.id, record.roleName)">{{$t('manage.account.authority.viewMember')}}</a-button>
      </template>
    </a-table>
    <!-- 表格 end-->
    ...
    <a-drawer :width="1000" :visible="formRoleDrawer.visible && !formRoleDrawer.editDept" @ok="handleRoleSubmitForm"
            ...
            <a-card :bordered="false" :title="$t('manage.account.role.roleInformation')">
              ...
              <a-form-item field="stopFlag" :label="$t('manage.account.role.enabledState')">
                <a-switch :checked-value="checkedValue" :unchecked-value="unCheckedValue"
                  ...
                  <template #unchecked>{{$t('global.outOfService')}}</template></a-switch>
              </a-form-item>
                ...
                <a-row>
                  <a-col>
                    <a-checkbox v-model="expandMenuValue"
                      @change="handleMenuTreeExpand">{{$t('global.expand')}}/{{$t('global.fold')}}</a-checkbox>
                    <a-checkbox v-model="checkedAllMenuValue"
                      @change="handleMenuTreeCheckedAll">{{$t('global.checkAll')}}/{{$t('global.checkOutAll')}}</a-checkbox>
                  </a-col>
                    ...
                    <div class="tree-container">
                      <div class="tree-box">
                        <a-tree ref="menuTreeRef" checked-strategy="all" :data="formRoleDrawer.menuTreeList"
                          :checkable="true" v-model:checked-keys="formRoleDrawer.formModel.menuIds" :fieldNames="{
        key: 'id',
        title: 'label',
        children: 'children'
    ...
    <!-- 数据权限 start -->
    <a-drawer :width="500" :visible="formRoleDrawer.visible && formRoleDrawer.editDept" @ok="handleDeptSubmitForm"
      @cancel="handleRoleFormCancel" :mask-closable="false">
                ...
                <a-row>
                  <a-col>
                    <a-checkbox v-model="expandDeptValue"
                      @change="handleDeptTreeExpand">{{$t('global.expand')}}/{{$t('global.fold')}}</a-checkbox>
                    <a-checkbox v-model="checkedAllDeptValue"
                      @change="handleDeptTreeCheckedAll">{{$t('global.checkAll')}}/{{$t('global.checkOutAll')}}</a-checkbox>
                  </a-col>
                  <a-col style="border-radius: 2px;border: 1px solid var(--color-neutral-4);">
                    <div class="tree-container">
                      <div class="tree-box">
    ...
    <a-modal width="auto" v-model:visible="authUserModalVisible" class="modal-box" :footer="false" title-align="start">
      ...
      <a-table row-key="id" :loading="loading" :bordered="{ wrapper: true, cell: true }"
        :pagination="authUserPagination" :columns="(tabColumnsOfUser as TableColumnData[])" :data="authUserRenderData">
        ...
        <template #operate="{ record }">
          <a-button size="small" type="text"
            @click="handleAuthUserCanleClick('取消授权', record.id, authUserSearchParams.roleId)">{{$t('manage.account.role.CancelAuthorization')}}</a-button>
        </template>
      </a-table>
    </a-modal>
    <!-- 查看成员弹出层 end -->
          ...
          <a-row :gutter="24">
            <a-col :span="10">
              <a-form-item field="userName" :label="$t('manage.account.role.userName')">
                <a-input v-model="authUserSearchParams.userName" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="10">
              <a-form-item field="phoneNumber" :label="$t('manage.account.role.phoneNumber')">
                <a-input v-model="authUserSearchParams.phonenumber" allow-clear />
              </a-form-item>
            </a-col>
...
<script lang="ts" setup>
  ...
  import { listRole, dataScope, getRole, updateRole, addRole, delRole, changeRoleStatus, allocatedUserList, unallocatedUserList, authUserCancel, authUserSelectAll } from "@/api/manage/account/role";
  ...
  const fetchRoleMenuTreeselect = async (id: any) => {
    try {
      const res = await roleMenuTreeselect(id);
      if (res.code == 200) {
  ...
  const handleDeptSubmitForm = async () => {
    ...
    console.log(res);
    if (!res) {
      console.log(formRoleDrawer.value)
      let data = {
  ...
  const handleAuthUserAddClickOk = async () => {
    try {
      ...
      let res = await authUserSelectAll({ roleId: authUserSearchParams.value.roleId, userIds: selectedKeys.value })
      notification(res)
      if (res.code == 200) {
