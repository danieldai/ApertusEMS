<template>
  <a-card class="content">
          ...
          <a-row :gutter="24">
            <a-col :span="6">
              <a-form-item field="nickName" :label="$t('manage.account.user.userName')">
                <a-input v-model="searchParams.nickName" :placeholder="$t('manage.account.user.userNamePlaceholder')" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="6">
              <a-form-item field="logicCode" :label="$t('manage.account.user.phoneNumber')">
                <a-input v-model="searchParams.mobile" :placeholder="$t('manage.account.user.numberPlaceholder')" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="6">
              <a-form-item :label="$t('manage.account.user.organizationChart')">
                <a-tree-select :placeholder="$t('manage.account.user.chartPlaceholder')" allow-search v-model="searchParams.deptId" :data="renderDeptData"
            ...
            <a-col :span="6">
              <a-form-item :label="$t('manage.account.user.userPost')">
                <a-select v-model="searchParams.postId" :placeholder="$t('manage.account.user.userPostPlaceholder')" allow-clear>
    ...
    <a-table row-key="id" :loading="loading" :bordered="{ wrapper: true, cell: true }" :pagination="pagination"
      :scroll="{ x: 1890 }" :columns="(tabColumns as TableColumnData[])" :data="renderData" @page-change="onPageChange"
      ...
      <template #operate="{ record }">
        ...
        <a-button size="small" type="text" status="success" @click="handleUpdateClick(record.id)">{{$t('global.edit')}}</a-button>
        <a-button size="small" type="text" status="success" @click="handleUpdatePwd(record)">{{$t('manage.account.user.changePassword')}}</a-button>
    ...
    <!-- 添加右弹层 start -->
    <a-drawer :loading="formDrawer.loading" :width="1000" :visible="formDrawer.visible" @ok="createDrawerHandleOk"
      ...
      <a-form ref="formRef" :rules="formDrawer.rules" :model="formDrawer.formModel" class="form" label-align="right"
        ...
        <a-row :gutter="24" class="row-mp-30">
          <a-col :span="14">
            <a-form-item field="nickName" :label="$t('manage.account.user.userName')" :validate-trigger="['change', 'input']">
              <a-input v-model="formDrawer.formModel.nickName" :max-length="20" allow-clear />
            </a-form-item>
            <a-form-item field="logonName" :label="$t('manage.account.user.loginAccount')" :validate-trigger="['change', 'input']">
              <a-input v-model="formDrawer.formModel.logonName" :max-length="20" allow-clear />
            </a-form-item>
            <a-form-item v-if="formDrawer.formModel.id == 0" field="logonPwd" :label="$t('manage.account.user.loginPassword')"
            ...
            <a-form-item v-if="formDrawer.formModel.id == 0" field="rawPassword" :label="$t('manage.account.user.passwordConfirmation')"
              :validate-trigger="['change', 'input']">
              <a-input-password autocomplete="new-password" v-model="formDrawer.formModel.rawPassword" :max-length="20"
                allow-clear></a-input-password>
            </a-form-item>

            <a-form-item field="mobile" :label="$t('manage.account.user.phoneNumber')" :validate-trigger="['change', 'input']">
            ...
            <a-form-item field="sex" :label="$t('manage.account.user.userGender')">
              <a-select v-model="formDrawer.formModel.sex" :max-length="20" allow-clear :placeholder="$t('manage.account.user.genderSelect')">
                <a-option v-for="dict in sys_user_sex" :key="dict.value" :label="dict.label"
                  :value="dict.value"></a-option>
              </a-select>
            </a-form-item>
            <a-form-item field="postIds" :label="$t('manage.account.user.position')" :validate-trigger="['change', 'input']">
            ...
            <a-form-item field="deptId" :label="$t('manage.account.user.department')" :validate-trigger="['change', 'input']">
              <a-tree-select :placeholder="$t('manage.account.user.departmentSelect')" allow-search v-model="formDrawer.formModel.deptId"
                :data="renderDeptData"></a-tree-select>
            </a-form-item>
            <a-form-item field="isSupper" :label="$t('manage.account.user.superManagement')">
              <a-switch :checked-value="1" :unchecked-value="0" v-model="formDrawer.formModel.isSupper"><template
                  #checked>{{$t('global.yes')}}</template><template #unchecked>{{$t('global.no')}}</template></a-switch>
            </a-form-item>
            ...
            <a-form-item field="personDes" :label="$t('manage.account.user.remarks')">
              <a-textarea v-model="formDrawer.formModel.personDes" allow-clear :placeholder="$t('manage.account.user.remarksPlaceholder')" />
            </a-form-item>
          </a-col>
          <a-col :span="1">
    ...
    <a-modal width="450px" v-model:visible="operateModalModel.visible" class="modal-box"
      ...
      <template #title>
        ...
        {{ operateModalModel.type < 2 ? $t('global.confirmTip') : $t('manage.account.user.tips') }} </template>
          ...
          <div v-else style="text-align: center;">
            <a-form :model="operateModalModel" layout="vertical">
              <a-form-item field="pwd" :label="operateModalModel.name">
                <a-input v-model="operateModalModel.pwd" :style="{ width: '450px' }" :placeholder="$t('manage.account.user.newPassword')"
                  allow-clear />
              </a-form-item>
            </a-form>
          </div>
...
const generateFormDrawerModel = () => {
  return {
    ...
    visible: false,
    rules: {
      nickName: [{ required: true, message: "请输入用户姓名" }, { minLength: 2, maxLength: 20, message: "用户姓名长度必须介于 2 和 20 之间" }],
      logonName: [{ required: true, message: "请输入登录账号" }, { minLength: 2, maxLength: 20, message: "登录账号长度必须介于 2 和 20 之间" }],
      logonPwd: [{ required: true, message: "请输入登录密码" }, { minLength: 5, maxLength: 20, message: "登录密码长度必须介于 5 和 20 之间" }],
      rawPassword: [{ required: true, message: "请输入登录密码" }, { minLength: 5, maxLength: 20, message: "登录密码长度必须介于 5 和 20 之间" }],
      mobile: [{ required: true, message: "请输入手机号码" }, { match: /^1[3|4|5|6|7|8|9][0-9]\d{8}$/, message: "请输入正确的手机号码" }],
...
const createDrawerHandleOk = async () => {
  ...
  if (!res) {
    console.log("form", formDrawer.value.formModel)
