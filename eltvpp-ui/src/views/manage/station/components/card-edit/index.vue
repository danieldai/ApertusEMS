    ...
    <div class="edit-wrapper" v-if="props.visible">
      ...
      <div class="edit-right">
        <div style="height:350px">
          <!--  修改的组件样式-->
          <component
              :is="data.staticType == 1 ? 'card-chart-pie-statistics' : data.staticType == 2 ? 'card-chart-combine' : EnumChartType[data.chartType]"
              :public="data"
              style="margin-top: 0">
          </component>
        </div>
...
const {datav_storage_type, datav_change_type, datav_percentage, datav_state_type} = proxy?.useDict("datav_storage_type",
    "datav_change_type", "datav_percentage", "datav_state_type");
...
const submit = async () => {
  ...
  if ([17, 18].includes(defaultVar.value.chartType)) {
    console.log(props.data.staticType)
    form.cardConfig.staticType = props.data.staticType;
  }
  ...
  console.log('传给接口的form', form)
  // return
  try {
    const res = await updateConfigCard(form);
    if (res.code == 200) {
...
const getDetailData = async () => {
  setLoading(true);
  try {
    const res = await getConfigCard(props.data);
    if (res?.code == 200) {
...
const getConfigDetail = async () => {
  try {
    setLoading(true);
    const res = await getConfigInfo(route.query.id);
    if (res.code === 200) {
