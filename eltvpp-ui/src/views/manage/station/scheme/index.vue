  ...
  <div>
    <a-card class="content">
            ...
            <a-row :gutter="16">
              <a-col :span="8">
                <a-form-item field="electricName" :label="$t('manage.station.scheme.priceName')">
                  <a-input v-model="searchModel.electricName" :placeholder="$t('manage.station.scheme.priceNamePlaceholder')" allow-clear />
                </a-form-item>
              </a-col>
              <a-col :span="8">
      ...
      <a-table row-key=id ref="tableRef" :loading="loading" :pagination="pagination" @page-change="onPageChange"
        ...
        :columns="(columns as TableColumnData[])" :data="renderData" :scroll="{ x: 1270 }">
        <!-- 地区 -->
        ...
        <template #operate="{ record }">
          <a-button size="small" type="text" status="success" @click="handleConfig(record)">{{$t('manage.station.scheme.configurePrice')}}</a-button>
          ...
          <a-button size="small" type="text" status="success" @click="handleRecalculate(record)">
            <template #icon>
              <icon-sync />
            </template>{{$t('manage.station.scheme.recalculationTariff')}}</a-button>
        </template>
      </a-table>

      <!-- 操作弹框 start -->
      <a-modal width="450px" :closable="false" :mask-closable="false" :esc-to-close="false" v-model:visible="operateModalModel.visible"
        class="modal-box">
        <template #title>
        ...
        <template #footer>
          <div style="text-align: center">
            <a-space>
              <a-button type="primary" status="danger" :disabled="operateModalModel.loading" @click="handleOperateModelCancle">{{$t('global.cancel')}}</a-button>
        ...
        <a-row :gutter="24">
          <a-col :span="24">
            <a-form ref="formRef" auto-label-width :model="formDrawer.formModel" label-align="right"
              ...
              <!-- 站点省市 -->
              <a-form-item field="electricAddressId" :label="$t('manage.station.scheme.applicableArea')">
                <a-cascader path-mode @change="addressChange" v-model="formDrawer.formModel.electricAddressId"
                  :virtual-list-props="{ height: 400 }" :options="addressOptions" :placeholder="$t('manage.station.scheme.areaSelect')"
                  :load-more="loadAddressMore" />
              </a-form-item>

              <!-- 用电分类 -->
              <a-form-item field="electricGroup" :label="$t('manage.station.scheme.electricityType')">
                <a-select v-model="formDrawer.formModel.electricGroup" :placeholder="$t('manage.station.scheme.electricitySelect')" allow-clear>
              ...
              <a-form-item field="electricStandard" :label="$t('manage.station.scheme.electricityStandard')">
                <a-select v-model="formDrawer.formModel.electricStandard" :placeholder="$t('manage.station.scheme.standardSelect')" allow-clear>
              ...
              <a-form-item field="voltageLevel" :label="$t('manage.station.scheme.voltageLevel')">
                <a-select v-model="formDrawer.formModel.voltageLevel" :placeholder="$t('manage.station.scheme.voltageLevelSelect')" allow-clear>
              ...
              <a-form-item field="effectYearDate" :label="$t('manage.station.scheme.effectiveYear')">
                <component is="a-year-picker" :placeholder="$t('manage.station.scheme.effectiveYearSelect')" format="YYYY" @change="onChange"
      ...
      <!-- 配置右弹层 start -->
      <a-drawer :width="1000" :visible="formDrawer.visibleConfig" @ok="handleConfigSubmitForm"
        ...
        <a-spin style="width: 100%;height: auto;" :tip="$t('global.loading')" :loading="formDrawer.configLoading">
          ...
          <a-row :gutter="24">
            <a-col :span="24">
              <a-form auto-label-width :model="formDrawer.formConfigModel" label-align="right" :rules="formDrawer.rules">
                <!-- 生效年份 -->
                ...
                <a-form-item :label="$t('manage.station.scheme.wattHour')" :content-flex="false" :merge-props="false">
                  <a-space direction="vertical" fill :size="16">
                    ...
                    <a-card v-for="(item, itemIndex) in formDrawer.formConfigModel.configList" class="content"
                      ...
                      <!-- 生效月份 -->
                      <a-form-item :field="`configList[${itemIndex}].effectMonth`" :label="$t('manage.station.scheme.effectiveMonth')">
                        <a-select :placeholder="$t('manage.station.scheme.effectiveMonthSelect')" v-model="item.effectMonthList"
                      ...
                      <!-- 电价类型 -->
                      <a-form-item :field="`configList[${itemIndex}].priceType`" :label="$t('manage.station.scheme.priceType')">
                        <a-radio-group v-model="item.priceType">
                          <a-radio v-for="dict in sys_electirc_price_type" :value="Number(dict.value)">{{ dict.label}}</a-radio>
                        </a-radio-group>
                      </a-form-item>
                      <!-- 全天电价 -->
                      ...
                      <a-form-item v-if="item.priceType == 1" :label="$t('manage.station.scheme.setUnitPrice')">
                        <a-space direction="vertical" fill :size="16">
                          <a-space>
                            <a-input-number v-model="item.jianPrice" allow-clear :min="0" :step="1" :precision="2">
                              <template #prefix>尖峰</template><template #suffix>元</template>
                            </a-input-number>
                            <a-input-number v-model="item.fengPrice" allow-clear :min="0" :step="1" :precision="2">
                              <template #prefix>峰段</template><template #suffix>元</template>
                            </a-input-number>
                            <a-input-number v-model="item.pingPrice" allow-clear :min="0" :step="1" :precision="2">
                              <template #prefix>平段</template><template #suffix>元</template>
                            </a-input-number>
                            <a-input-number v-model="item.guPrice" allow-clear :min="0" :step="1" :precision="2">
                              <template #prefix>谷段</template><template #suffix>元</template>
                            </a-input-number>
                            <a-input-number v-model="item.shenPrice" allow-clear :min="0" :step="1" :precision="2">
                              <template #prefix>深谷</template><template #suffix>元</template>
                            </a-input-number>
                          </a-space>
                          ...
                          <a-table :bordered="{ wrapper: true, cell: true }" :pagination="false"
                            :data="item.seasonalRange" :columns="(rangeColumns as TableColumnData[])">
                            <!-- 开始时间 -->
                            ...
                            <template #chargePrice="{ record }">
                              <a-input-number v-model="record.chargePrice" allow-clear :min="0" :step="1" :precision="2">
...
import { getPriceScheme, addPriceScheme, updatePriceScheme, delPriceScheme, listPriceScheme, changePriceSchemeStatus, recalculateSeasonalPriceBySchemeId } from "@/api/system/price-scheme";
...
const { sys_use_electric_categroy, sys_use_electric_categroy_type, sys_voltage_level, sys_electirc_price_type, sys_seasonal_type } = proxy?.useDict("sys_use_electric_categroy", "sys_use_electric_categroy_type", "sys_voltage_level", "sys_electirc_price_type", "sys_seasonal_type");
//******* 这里编写动态获取下拉列表 end ******
...
const rangeColumns = computed<TableColumnData[]>(() => [
  {
    title: "开始时间",
    dataIndex: "startTime",
    slotName: "startTime",
...
const handleOperateModelOk = async () => {
  ...
  operateModalModel.value.loading = true;
  try {
