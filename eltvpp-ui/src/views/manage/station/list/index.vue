        ...
        <a-form ref="formRef" :model="formDrawer.formModel" label-align="right" :rules="formDrawer.rules"
          ...
          <a-row :gutter="24" class="row-mp-30">
            <a-col :span="12">
              ...
              <!-- 站点编号 -->
              <a-form-item field="stationSn" class="star-from-item" disabled :label="$t('manage.station.list.stationNumber')">
              ...
              <a-form-item field="picUrl" :label="$t('manage.station.list.sitePhoto')">
                <a-upload response-url-key="url" :limit="1" list-type="picture-card" :file-list="formDrawer.fileList"
                          image-preview
                          :action="(`${baseUrl}/file/upload`)" @success="handleUploadSuccess" :headers="headers"/>
              </a-form-item>
            ...
            <a-col :span="12">
              ...
              <a-form-item field="coordinate" :label="$t('manage.station.list.coordinates')">
                <a-input-search v-model="formDrawer.formModel.coordinate" @search="handleSelectLocation" search-button>
              ...
              <a-form-item field="schemeId" :label="$t('manage.station.list.priceStandard')">
                <a-select v-model="formDrawer.formModel.schemeId" :placeholder="$t('manage.station.list.tariffSelect')"
                          allow-clear>
                  <a-option v-for="(item, index) in renderPriceSchemeData" :key="index" :value="item.id">{{
                      item.electricName
                    }}
                  </a-option>
                </a-select>
...
const handleOperateModelOk = async () => {
  let result = await changeStationStatus(operateModalModel.value.id, operateModalModel.value.value);
  notification(result);
  if (result.code == 200) {
...
const handleUpdate = async (record: any) => {
  ...
  if (result.code == 200) {
    result.data.parentId = result.data.parentId;
