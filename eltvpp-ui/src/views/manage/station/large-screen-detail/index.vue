          ...
          <a-row :gutter="24">
            ...
            <a-col :span="12" style="position:relative;">
              <a-form-item label="壁纸">
                <up-load :imgUrl="pageConfig.background.url" @upload="imgUpload($event, 'background')"
                         class="upload-view"></up-load>
              </a-form-item>
            </a-col>
          </a-row>
          <a-row :gutter="24">
            ...
            <a-col :span="24">
              <a-form-item label="核心数据">
                <a-radio-group v-model="defaultRadio">
                  <a-radio v-for="item in pageConfig.coreMainData" :key="item.id" :value="item.key">
                    {{ item.label }}
                  </a-radio>
                </a-radio-group>
              </a-form-item>
...
import {coreSubStatOption, previewList, reduceArr} from './index';
  ...
  coreMainData: [
    {
      ...
      id: 4,
      label: "总储能量",
      key: "total_storage_power",
      value: false,
    }, {
      id: 5,
      label: "总减排量",
      key: "total_emission_reduction",
      value: false,
    }
  ],
  coreSubStatData: {
...
const getConfigDetail = async (id: string) => {
  try {
    setLoading(true);
    const res = await getConfigInfo(id);
    if (res.code === 200) {
      form.value = res.data;
      pageConfig.value = res.data.pageConfig === "{}" ? JSON.parse(res.data.pageConfigPre) : JSON.parse(res.data.pageConfig)
      getCardListAll();
...
const setEntOption = async () => {
  const res = await listEnterpriseAll();
  ...
  formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      ...
      if (type === 'preview') {
        ...
        router.push({path: '/manage/station/largeScreenConfig'});
      }
...
const handleCancel = () => {
  router.push({
    path: '/manage/station/largeScreenConfig'
  });
}
...
const getCardListAll = async () => {
  ...
  try {
    setLoading(true);
    const res = await getCardList({dashboardConfigId: route.query.id});
    if (res?.code == 200) {
