    ...
    <a-table row-key="id" ref="tableRef" :loading="loading" :bordered="{ wrapper: true, cell: true }"
      ...
      <template #operate="{ record }">
        ...
        <a-button size="small" type="text" status="danger"
          @click="handleDelete('删除', record.id)">{{$t('global.delete')}}</a-button>
        <a-button size="small" type="text" status="success"
          @click="handleDetail(record)">{{$t('global.detail')}}</a-button>
        <a-button size="small" type="text" status="success"
          @click="createToken(record)">{{$t('manage.interface.apply.refreshToken')}}</a-button>
      </template>
    </a-table>
    <!-- 表格 end-->

    <!-- 编辑抽屉 start -->
    <a-drawer :width="750" :visible="formData.visible" @ok="createDrawerHandleOk" @cancel="createDrawerHandleCancel"
      :mask-closable="false">
      <template #title>
            ...
            <a-form ref="formRef" :model="formData.formModel" class="form" auto-label-width>
              ...
              <a-form-item field="appId" :label="$t('manage.interface.apply.uniqueIdentification')">
                ...
                <a-button type="primary" @click="createIdentification"
                  style="margin-left: 10px">{{$t('manage.interface.apply.generateIdentification')}}</a-button>
              </a-form-item>
              ...
              <a-form-item field="appKey" :label="$t('manage.interface.apply.accountNumber')">
                ...
                <a-button type="primary" @click="createAccount"
                  style="margin-left: 10px">{{$t('manage.interface.apply.generateAccount')}}</a-button>
              </a-form-item>
              ...
              <a-form-item field="appSecret" :label="$t('manage.interface.apply.secretKey')">
                ...
                <a-button type="primary" @click="createPassword"
                  style="margin-left: 10px">{{$t('manage.interface.apply.generateKey')}}</a-button>
              </a-form-item>
...
<script lang="ts" setup>
  ...
  import { listApply, getApply, addApply, updateApply, delApply, addAccount, addSecret, addToken } from "@/api/manage/interface/apply";
