    ...
    <a-card class="content">
            ...
            <a-row :gutter="16">
              <a-col :span="8">
                <a-form-item field="channelName" :label="$t('manage.channel.list.channelName')">
                  <a-input v-model="searchModel.channelName" :placeholder="$t('manage.channel.list.fuzzyQuery')" allow-clear />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item field="registrationCode" :label="$t('manage.channel.list.registrationCode')">
                  <a-input v-model="searchModel.registrationCode" :placeholder="$t('manage.channel.list.fuzzyQuery')" allow-clear />
                </a-form-item>
              </a-col>
              <!-- <a-col :span="8">
      ...
      <a-table row-key="id" ref="tableRef" :loading="loading" :pagination="pagination" :scroll="{ x: 1350 }"
        ...
        <template #operate="{ record }">
          ...
          <a-button size="small" type="text" status="success" @click="handleUpdate(record.id)">{{$t('global.edit')}}</a-button>
          <a-button size="small" type="text" status="success" @click="handleDetail(record.id)">{{$t('global.detail')}}
          </a-button>
          <a-button size="small" type="text" status="success" @click="handleToDevice(record.id)">{{$t('manage.channel.list.equipment')}}
          </a-button>
        </template>
      </a-table>
      <!-- 表格内容 end-->
        ...
        <a-row :gutter="24">
          <a-col :span="24">
            <a-form ref="formRef" auto-label-width :model="formDrawer.formModel" label-align="right"
              ...
              <!-- 通道名称 -->
              <a-form-item field="channelName" :label="$t('manage.channel.list.channelName')" :validate-trigger="['change', 'blur']">
              ...
              <!-- 接入方式 -->
              <a-form-item field="accessType" :label="$t('manage.channel.list.accessMode')" :validate-trigger="['change', 'blur']">
                <a-radio-group v-model="formDrawer.formModel.accessType">
                  <a-radio v-for="dict in sys_channel_type" :key="dict.value" :value="Number(dict.value)">{{ dict.label
                    }}</a-radio>
                </a-radio-group>
              </a-form-item>
              ...
              <div v-if="formDrawer.formModel.accessType == 1">
                <!-- 通讯注册码 -->
                <a-form-item field="registrationCode" :label="$t('manage.channel.list.communicationCode')" :validate-trigger="['change', 'blur']">
                ...
                <!-- 注册回应码 -->
                <a-form-item field="registrationRsp" :label="$t('manage.channel.list.responseCode')" :validate-trigger="['change', 'blur']">
              ...
              <div v-if="formDrawer.formModel.accessType == 2">
                ...
                <!-- 连接地址 -->
                <a-form-item field="connIp" :label="$t('manage.channel.list.connectionAddress')" :validate-trigger="['change', 'blur']">
                  <a-input v-model="formDrawer.formModel.connIp" allow-clear />
                </a-form-item>

                <!-- 连接端口 -->
                <a-form-item field="connPort" :label="$t('manage.channel.list.connectingPort')" :validate-trigger="['change', 'blur']">
                  <a-input-number :min="0" :max="65535" :precision="0" v-model="formDrawer.formModel.connPort"
                    allow-clear />
                </a-form-item>

                <!-- 连接帐号 -->
                <a-form-item field="connUsername" :label="$t('manage.channel.list.connectionAccount')" :validate-trigger="['change', 'blur']">
                  <a-input v-model="formDrawer.formModel.connUsername" allow-clear />
                </a-form-item>

                <!-- 连接密码 -->
                <a-form-item field="connPwd" :label="$t('manage.channel.list.connectionPassword')" :validate-trigger="['change', 'blur']">
                  <a-input v-model="formDrawer.formModel.connPwd" allow-clear />
                </a-form-item>

                <!-- 订阅主题 -->
                <a-form-item field="subscribeTopic" :label="$t('manage.channel.list.topic')" :validate-trigger="['change', 'blur']">
                  <a-input v-model="formDrawer.formModel.subscribeTopic" allow-clear />
                </a-form-item>

                <!-- 发布主题 -->
                <a-form-item field="publishTopic" :label="$t('manage.channel.list.releaseTopic')" :validate-trigger="['change', 'blur']">
                  <a-input v-model="formDrawer.formModel.publishTopic" allow-clear />
                </a-form-item>
              </div>

              <!-- 上报地址 -->
              <a-form-item field="ipAddress" :label="$t('manage.channel.list.reportAddress')" :validate-trigger="['change', 'blur']">
                <a-input v-model="formDrawer.formModel.ipAddress" allow-clear />
              </a-form-item>

              <!-- 上报端口 -->
              <a-form-item field="port" :label="$t('manage.channel.list.reportingPort')" :validate-trigger="['change', 'blur']">
                <a-input-number :min="0" :max="65535" :precision="0" v-model="formDrawer.formModel.port" allow-clear />
              </a-form-item>

              <!-- 连接超时 -->
              <a-form-item field="timeout" :label="$t('manage.channel.list.connectionTimeout')" :validate-trigger="['change', 'blur']">
                <a-input-number :precision="0" v-model="formDrawer.formModel.timeout" allow-clear><template
                    #append>{{$t('manage.channel.list.seconds')}}</template></a-input-number>
              </a-form-item>

              <!-- 调度周期 -->
              <a-form-item field="scheduleInterval" :label="$t('manage.channel.list.schedulingCycle')" :validate-trigger="['change', 'blur']">
                <a-input-number :precision="0" v-model="formDrawer.formModel.scheduleInterval" allow-clear><template
                    #append>{{$t('manage.channel.list.ms')}}</template></a-input-number>
              </a-form-item>

              <!-- 连接报警 -->
              <a-form-item field="connectAlarm" :label="$t('manage.channel.list.connectionAlarm')" :validate-trigger="['change', 'blur']">
                <a-switch :checked-value="checkedValue" :unchecked-value="unCheckedValue"
                  v-model="formDrawer.formModel.connectAlarm"><template #checked>{{$t('global.on')}}</template><template
                    #unchecked>{{$t('global.off')}}</template></a-switch>
              </a-form-item>
...
import { addChannel, changeChannelStatus, getChannel, listChannel, updateChannel } from '@/api/system/channel';
...
const handleOperateModelOk = async () => {
  let result = await changeChannelStatus(operateModalModel.value.id, operateModalModel.value.value);
  notification(result);
  if (result.code == 200) {
