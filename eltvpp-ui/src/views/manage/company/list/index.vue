...
<template>
  <div>
    <a-card class="content">
      ...
      <!-- 编辑、添加右弹层 start -->
      <a-drawer :width="900" :visible="formDrawer.visible" @ok="handleSubmitForm" @cancel="handleFormCancel"
        :mask-closable="false">
        ...
        <a-spin style="width: 100%;height: auto;" :loading="formDrawer.loading">
            ...
            <a-row :gutter="24">
              <a-col :span="12">
                ...
                <a-form-item field="companyName" :label="$t('manage.company.list.companyName')" :validate-trigger="['change', 'blur', 'input']">
                  <a-input v-model="formDrawer.formModel.companyName" allow-clear :placeholder="$t('manage.company.list.companyNameMaximum')" />
                </a-form-item>
          ...
          children: 'children'
                ...
                <a-form-item field="homePage" :label="$t('manage.company.list.companyWebsite')" :validate-trigger="['change', 'blur', 'input']">
                  <a-input v-model="formDrawer.formModel.homePage" allow-clear />
                </a-form-item>
...
const handleDetail = async (id: any) => {
  ...
  try {
    ...
    if (res.code == 200) {
      ...
      let parentName = '';
      let resParent = await getCompany(res.data.parentId);
      if (resParent.code == 200 && resParent.data) {
        parentName = resParent.data.companyName;
      }
      let industryGroupName = '';
      let resGroup = await getGroup(res.data.industryGroupId);
